...
class TestCrossRefSearcher(unittest.TestCase):
    ...
    def test_search(self):
        ...
        for i, paper in enumerate(papers, 1):
            ...
            print(
                f"   Published: {paper.published_date.year if paper.published_date else 'N/A'}"
            )
    ...
    def test_download_pdf_not_supported(self):
        ...
        self.assertIn(
            "CrossRef does not provide direct PDF downloads", str(context.exception)
        )
    ...
    def test_user_agent_header(self):
        ...
        self.assertIn(
            "paper-search-mcp", self.searcher.session.headers.get("User-Agent", "")
        )
