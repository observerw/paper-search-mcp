...
class TestPaperSearchServer(unittest.TestCase):
    ...
    def test_download_arxiv_from_search(self):
        ...
        search_results = asyncio.run(
            server.search_arxiv("machine learning", max_results=10)
        )
        ...
        for paper in search_results:
            ...
            self.assertIsInstance(
                result, str, f"Result for {paper_id} should be a file path"
            )
            self.assertTrue(
                result.endswith(".pdf"),
                f"Result for {paper_id} should be a PDF file path",
            )
            self.assertTrue(
                os.path.exists(result), f"PDF file for {paper_id} should exist on disk"
            )
